...
**Within a world**: The enrichment reviewer and playtest auditor for the **same** world should run **in parallel** — they are independent reviews and there is no reason to serialize them. Launch both at the same time, collect both results, then integrate all feedback together.
...
## VERIFY REVIEW RESULTS — DO NOT TRUST SUMMARIES

When a reviewer agent completes, you MUST:

1. **Read the actual review file** it wrote to disk. Do not rely on the task notification summary alone. Summaries are lossy — they may omit P0 issues, mischaracterize findings, or claim a pass when the file says otherwise.
...
3. **Do not declare a world "DONE" unless the review files on disk confirm PASS with no P0/P1 issues.** A task notification saying "PASS" is not sufficient — read the file.
